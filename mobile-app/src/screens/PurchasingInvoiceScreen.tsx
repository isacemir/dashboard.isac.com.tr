...
// FATURA_DURUMU sayısal kod → label
...
export const PurchasingInvoiceScreen: React.FC = () => {
  ...
  const rawData = useMemo(() => DataService.getPurchasingInvoices(), []);
  ...
  const tamamlanan = useMemo(() => rawData.filter(d => d.FATURA_DURUMU === 5 || d.FATURA_DURUMU === 3).length, [rawData]);
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          <View style={styles.cardIconWrap}>
            <MaterialIcons name="request-quote" size={20} color="#006290" />
          </View>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>{excelDate(item.TARIHI)} · {item.YIL || ''}/{item.AY || ''}</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <Header title="ISAC Sense Digital" subtitle="Satınalma Faturaları" onMenuPress={() => setDrawerOpen(true)} onNotificationPress={() => (navigation as any).navigate("Notifications")} onProfilePress={() => (navigation as any).navigate("Profile")} />
      <View style={styles.summaryRow}>
        <View style={[styles.summaryCard, { backgroundColor: '#e0f2fe' }]}>
          <MaterialIcons name="request-quote" size={16} color="#006290" />
      ...
      <View style={styles.searchWrap}>
        ...
        <TextInput style={styles.searchInput} placeholder="Fatura no, tedarikçi, ürün veya marka ara..." placeholderTextColor="#94a3b8" value={search} onChangeText={setSearch} />
