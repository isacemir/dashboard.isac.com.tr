...
export const SalesScreen: React.FC = () => {
  ...
  const regionParam: string = route.params?.region || '';
  ...
  const rawData = useMemo(() => DataService.getSalesReports(), []);

  const filtered = useMemo(() => {
    let data = rawData;
    if (regionParam) data = data.filter(item => String(item.GRUBU || '') === regionParam);
    if (!search.trim()) return data;
    const q = search.toLowerCase();
    return data.filter(item =>
      String(item.TICARI_UNVANI || '').toLowerCase().includes(q) ||
      String(item.FATURA_NO || '').toLowerCase().includes(q) ||
      String(item.STOK_ADI || '').toLowerCase().includes(q) ||
      String(item.MARKASI || '').toLowerCase().includes(q)
    );
  }, [rawData, search, regionParam]);

  const totalEur = useMemo(() => rawData.reduce((s, d) => s + Number(d.DVZ_IND_TUTAR || 0), 0), [rawData]);
  const tamamlanan = useMemo(() => rawData.filter(d => d.FATURA_DURUMU === 3 || d.FATURA_DURUMU === 5).length, [rawData]);
  const iptal = useMemo(() => rawData.filter(d => d.IPTAL === 1 || d.FATURA_DURUMU === 6).length, [rawData]);
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          <View style={styles.cardIconWrap}>
            <MaterialIcons name="receipt" size={20} color="#006290" />
          </View>
          <View style={styles.cardMeta}>
            <Text style={styles.cardTitle} numberOfLines={1}>{item.TICARI_UNVANI || '—'}</Text>
            <Text style={styles.cardSub}>{item.FATURA_NO || '—'}</Text>
          </View>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>{item.ILI_1 || '—'}{item.ILCESI_1 ? ` / ${item.ILCESI_1}` : ''} · {item.GRUBU || ''}</Text>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <Header title="ISAC Sense Digital" subtitle={regionParam ? `Satış · ${regionParam}` : 'Satış Raporları'} onMenuPress={() => setDrawerOpen(true)} onNotificationPress={() => (navigation as any).navigate("Notifications")} onProfilePress={() => (navigation as any).navigate("Profile")} showNotification showProfile />
      <View style={styles.summaryRow}>
        <View style={[styles.summaryCard, { backgroundColor: '#e0f2fe' }]}>
          <MaterialIcons name="receipt" size={16} color="#006290" />
        ...
        <View style={[styles.summaryCard, { backgroundColor: '#dcfce7' }]}>
          ...
          <Text style={[styles.summaryNum, { color: '#15803d' }]}>{tamamlanan}</Text>
          <Text style={styles.summaryLabel}>Onaylı</Text>
        </View>
        <View style={[styles.summaryCard, { backgroundColor: '#fee2e2' }]}>
          <MaterialIcons name="cancel" size={16} color="#dc2626" />
          <Text style={[styles.summaryNum, { color: '#dc2626' }]}>{iptal}</Text>
          <Text style={styles.summaryLabel}>İptal</Text>
        </View>
      ...
      <View style={styles.searchWrap}>
        ...
        <TextInput style={styles.searchInput} placeholder="Müşteri, fatura no, ürün veya marka ara..." placeholderTextColor="#94a3b8" value={search} onChangeText={setSearch} />
