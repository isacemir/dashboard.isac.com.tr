...
const STATUS_STYLE: Record<string, { bg: string; text: string }> = {
  'Açık':           { bg: '#e0f2fe', text: '#006290' },
  'Kapalı':         { bg: '#dcfce7', text: '#15803d' },
  'İptal':          { bg: '#fee2e2', text: '#dc2626' },
  'Muhasebelendi':  { bg: '#dcfce7', text: '#15803d' },
  'Bekliyor':       { bg: '#fef9c3', text: '#a16207' },
  'Kısmi Teslim':   { bg: '#e0f2fe', text: '#006290' },
};

const TAB = ['Dashboard', 'Sales', 'Purchasing', 'Stock', 'CRM'];

export const OrderReportScreen: React.FC = () => {
  ...
  const rawData = useMemo(() => DataService.getOrderReports(), []);
  ...
  const filtered = useMemo(() => {
    ...
    return rawData.filter(item =>
      ...
      String(item.SIPARIS_NO || '').toLowerCase().includes(q) ||
  ...
  const counts = useMemo(() => {
    const durumlar = rawData.reduce((acc: Record<string, number>, d) => {
      const k = String(d.SIPARIS_DURUMU || 'Belirsiz');
      acc[k] = (acc[k] || 0) + 1;
      return acc;
    }, {});
    return durumlar;
  ...
  const renderItem = ({ item, index }: { item: any; index: number }) => {
    const s = statusStyle(item.SIPARIS_DURUMU);
    const tutar = Number(item.KPB_IND_TUTAR || 0);
    const eurTutar = Number(item.DVZ_IND_TUTAR || 0);
    return (
      <View style={styles.card}>
        {/* Kart Başlık */}
        <View style={styles.cardHeader}>
          <View style={styles.cardIconWrap}>
            <MaterialIcons name="receipt-long" size={20} color="#006290" />
          </View>
          <View style={styles.cardMeta}>
            <Text style={styles.cardTitle} numberOfLines={1}>{item.TICARI_UNVANI || '—'}</Text>
            <Text style={styles.cardSub}>#{item.SIPARIS_NO || '—'} · {item.SIPARIS_TURU || ''}</Text>
          </View>
          <View style={[styles.badge, { backgroundColor: s.bg }]}>
            <Text style={[styles.badgeText, { color: s.text }]}>{item.SIPARIS_DURUMU || '—'}</Text>
          </View>
        </View>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>Sipariş: {excelDate(item.TARIHI)}</Text>
          <Text style={styles.sep}>·</Text>
          <Text style={styles.infoText}>Vade: {excelDate(item.VADESI)}</Text>
        </View>
        ...
        <View style={styles.metricsRow}>
          ...
          <View style={styles.metric}>
            ...
            <Text style={[styles.metricValue, { color: '#15803d' }]}>{item.MIKTARI_TESLIM || '0'}</Text>
          </View>
          <View style={styles.metricDivider} />
          <View style={styles.metric}>
            <Text style={styles.metricLabel}>Kalan</Text>
            <Text style={[styles.metricValue, { color: item.MIKTARI_KALAN ? '#dc2626' : '#64748b' }]}>
              {item.MIKTARI_KALAN || '0'}
            </Text>
          </View>
          <View style={styles.metricDivider} />
          <View style={styles.metric}>
            <Text style={styles.metricLabel}>Tutar</Text>
            <Text style={[styles.metricValue, { color: '#006290', fontSize: 11 }]}>
              {eurTutar > 0 ? `€${eurTutar.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}` : `₺${tutar.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}`}
            </Text>
          </View>
        </View>
      </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <Header title="ISAC Sense Digital" subtitle="Sipariş Raporları" onMenuPress={() => setDrawerOpen(true)} onNotificationPress={() => (navigation as any).navigate("Notifications")} onProfilePress={() => (navigation as any).navigate("Profile")} />

      {/* Özet */}
      <View style={styles.summaryRow}>
        <View style={[styles.summaryCard, { backgroundColor: '#e0f2fe' }]}>
          <MaterialIcons name="receipt-long" size={16} color="#006290" />
          <Text style={[styles.summaryNum, { color: '#006290' }]}>{rawData.length}</Text>
          <Text style={styles.summaryLabel}>Toplam</Text>
        </View>
        {Object.entries(counts).slice(0, 3).map(([durum, sayi]) => {
      ...
      <View style={styles.searchWrap}>
        ...
        <TextInput
          ...
          placeholder="Müşteri, sipariş no, ürün veya marka ara..."
...
const styles = StyleSheet.create({
  ...
  summaryNum: { fontSize: isSmallScreen ? 15 : 17, fontWeight: '800' },
