...
if (!$data || !isset($data['target']) || !isset($data['data'])) {
    http_response_code(400);
    echo json_encode(['ok' => false, 'error' => 'Geçersiz veri formatı.']);
    exit;
}
...
if (!is_array($rows) || empty($rows)) {
    http_response_code(400);
    echo json_encode(['ok' => false, 'error' => 'Veri bulunamadı veya boş.']);
    exit;
}

// Process data and build KPI + Charts (same logic as original upload.php)
try {
    ...
    echo json_encode(['ok' => false, 'error' => 'İşleme hatası: ' . $e->getMessage()]);
}
