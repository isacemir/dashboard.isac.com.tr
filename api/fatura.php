...
if (!empty($p['fat_durumu']))  { $where[]="FATURA_DURUMU = ?";     $params[]=[$p['fat_durumu'],SQLSRV_PARAM_IN]; }
if(($p['iptal_gizle']??'')==='1') $where[]="IPTAL = 0";

$sql="SELECT FATURA_DURUMU,IPTAL,CARIKODU,TICARI_UNVANI,VERGI_NO,
      GRUBU,ARA_GRUBU,ALT_GRUBU,MARKASI,MODELI,ILI_1,ILCESI_1,
      FATURA_NO,TARIHI,STOKKODU,STOK_ADI,MIKTARI,
      DVZ_IND_FIYAT,DVZ_IND_TUTAR,DOVIZ_BIRIMI_1,
      IKINCI_DVZ_IND_FIYAT,IKINCI_DVZ_IND_TUTAR,YIL,AY
      FROM TRV_SATIS_RAPORU WHERE ".implode(" AND ",$where)." ORDER BY TARIHI DESC";
