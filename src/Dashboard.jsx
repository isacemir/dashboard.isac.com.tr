...
import KullaniciDetayTab from "./tabs/KullaniciDetayTab.jsx";
...
export default function Dashboard() {
  ...
  useEffect(()=>{
    ...
    const kullaniciAdi = urlParams.get('adi');
    const tab = urlParams.get('tab');
    const bolge = urlParams.get('bolge');
    ...
    if (cariKodu && cariUnvan) {
      ...
      setKullaniciDetay({ adi: decodeURIComponent(kullaniciAdi) });
    } else if (tab) {
      setActive(tab);
      // Filtreleri state'e ekle
      const filters = {};
      if (bolge) filters.bolge = decodeURIComponent(bolge);
  ...
  // Eğer kullanıcı detay sayfası ise
  if (kullaniciDetay) {
    return (
      <div style={{minHeight:"100vh",background:"#f1f5f9",fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif"}}>
        <header style={{background:"#fff",borderBottom:"1px solid #e2e8f0",padding:"0 2rem",display:"flex",alignItems:"center",justifyContent:"space-between",height:54,position:"sticky",top:0,zIndex:100,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              ...
              <div style={{fontSize:10,color:"#94a3b8",letterSpacing:"0.05em"}}>KULLANICI DETAY</div>
            </div>
          </div>
        ...
        <main style={{padding:"1.5rem 2rem",maxWidth:1700,margin:"0 auto"}}>
          <KullaniciDetayTab kullaniciAdi={kullaniciDetay.adi} />
        </main>
      </div>
    );
  }
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f1f5f9",fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif"}}>
      ...
      <main style={{padding:"1.5rem 2rem",maxWidth:1700,margin:"0 auto"}}>
        {AT && <AT.component urlFilters={urlFilters} />}
      </main>
    </div>
  );
}
